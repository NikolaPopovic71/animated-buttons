...
export function RocketButton({ onComplete }: RocketButtonProps) {
  const { state, trigger, reset } = useButtonAnimation({ duration: 5000, onComplete })
  ...
  const icon = (
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" aria-hidden="true">
      ...
      <g className={`${styles.flame} ${styles[state]}`} transform="translate(14,23)">
        <ellipse cx="4" cy="3" rx="4" ry="5" fill="#ff9800" opacity="0.9"/>
        <ellipse cx="4" cy="4" rx="2" ry="3" fill="#ffeb3b"/>
      </g>
      {(state === 'running' || state === 'done') && (
        <>
          <circle cx="6"  cy="8"  r="1.2" fill="white" opacity="0.6" className={styles.starDot}/>
          <circle cx="30" cy="14" r="0.9" fill="white" opacity="0.5" className={`${styles.starDot} ${styles.d1}`}/>
          <circle cx="22" cy="4"  r="1"   fill="white" opacity="0.7" className={`${styles.starDot} ${styles.d2}`}/>
          <circle cx="4"  cy="22" r="0.8" fill="white" opacity="0.4" className={`${styles.starDot} ${styles.d3}`}/>
        </>
      )}
    </svg>
  )
